...
В проект было добавлено объектное хранилище MinIO для работы с файлами. Реализована загрузка и скачивание файлов с потоковой обработкой (Streams) — файлы читаются и передаются частями по 8 КБ, без полной загрузки в память. Реализовано обновление профиля через POST /profile с возможностью установки аватара с проверкой прав владения файлом. Добавлена валидация загружаемых файлов: максимальный размер 10 МБ и разрешённые MIME-типы (изображения).
